 ...
 * @param   options Specifies whether to check for missing or for extra.
 *                  Optional. Default to all options being true.
 *                  Use the format:
 *                      {
 *                          noMissing: true || false,
 *                          noExtra: true || false
 *                      }
 * @param   prefix  Used for nested pathing to clearly see the
...
module.exports.assertObjectSchema
    ...
    if (config.noExtra) {
        ...
        _.forIn(obj, function (value, key) {
            ...
            if (_.isObject(value) && !_.isEmpty(value)
                    && _.has(test, key) && _.isObject(_.get(test, key))) {
                // If the value is also an object, we need to check it as well
                assertObjectSchema(value, _.get(test, key), config, key + '.');
            }
        });
    }
    ...
    if (config.noMissing) {
        ...
        _.forIn(test, function (value, key) {
            ...
            if (_.isObject(value) && !_.isEmpty(value)
                    && _.has(obj, key) && _.isObject(_.get(obj, key))) {
                // If the value is also an object, we need to check it as well
                assertObjectSchema(_.get(obj, key), value, config, key + '.');
            }
        });
    }
};
